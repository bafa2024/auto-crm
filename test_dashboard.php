<?php
// test_dashboard.php - Comprehensive Testing Dashboard
    ...
    switch ($testType) {
        ...
        case 'signup':
            try {
                ...
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'https://autocrm.regrowup.ca/api.php/api/auth/register');
        ...
        case 'login':
            try {
                ...
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'https://autocrm.regrowup.ca/api.php/api/auth/login');
                curl_setopt($ch, CURLOPT_POST, true);
                curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($testData));
                curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
runTest('API', 'Fallback Endpoint', function() {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://autocrm.regrowup.ca/api.php');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_TIMEOUT, 10);
    ...
    if ($httpCode == 404) {
        return ['message' => 'Fallback API endpoint accessible'];
    }
    throw new Exception("Fallback API returned HTTP $httpCode");
});

runTest('Security', 'HTTPS', function() {
    if (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') {
        return ['message' => 'Running on HTTPS'];
    }
    return ['message' => 'Running on HTTP (consider HTTPS for production)'];
});
...
<head>
    ...
    <title>AutoDial Pro CRM - Testing Dashboard</title>
...
<body>
    <div class="container-fluid mt-4">
        <div class="row">
            <div class="col-12">
                <h1 class="mb-4">
                    <i class="bi bi-speedometer2"></i> AutoDial Pro CRM - Testing Dashboard
                </h1>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <button class="btn btn-success btn-test" onclick="runManualTest('signup')">
                                        <i class="bi bi-person-plus"></i> Test Signup
                                    </button>
                                </div>
                                <div class="col-md-3">
                                ...
                                <div class="col-md-3">
                                    <button class="btn btn-warning btn-test" onclick="runAllTests()">
    ...
    <script>
        ...
        function runAllTests() {
            const tests = ['database', 'signup', 'login'];
